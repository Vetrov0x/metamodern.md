...
summary: "Notre modèle le plus cher a obtenu 6/12 en revue de code. Un modèle intermédiaire avec une instruction a obtenu 11/12."
pubDate: 2026-02-17
...
## Plus d'agents, moins de résultats — sur les tâches à fichier unique

C'était le résultat inconfortable sur les tâches à fichier unique.
...
**Le tableau s'inverse lorsque la tâche couvre plusieurs fichiers.** Une expérience de suivi sur un pipeline de production à trois scripts (détection de signaux → génération de brouillons → publication) a renversé le résultat : Équipe 9/12, Solo 7/12 — un delta de +2 en faveur de l'équipe. Les agents solo ont chuté de 11/12 sur un seul fichier à 7/12 sur trois fichiers — une pénalité de −4 pour la complexité que les équipes ont absorbée grâce à la spécialisation par fichier. Chaque agent possédait un composant et détectait des chaînes d'exploitation inter-scripts qu'une seule fenêtre de contexte ratait.

Le seuil se situe approximativement à trois fichiers avec un flux de données inter-fichiers réel. En dessous, les équipes ajoutent des coûts de coordination sans bénéfice de parallélisme. Au-dessus, la spécialisation justifie les coûts.
...
Pas d'essaims d'agents pour les tâches à fichier unique — les équipes deviennent utiles à partir de trois fichiers ou plus avec de véritables interfaces inter-fichiers. Pas d'injection de compétences. Pas de modèles coûteux. Cette configuration a obtenu 11 à 12/12 sur chaque type de tâche que nous avons testé.
...
*Cet article s'appuie sur quatre séries d'expériences : H-MATRIX (23 expériences), la recherche VERIFY préalable (36 expériences), la validation étendue des Lacunes Mendeleïev (4 expériences) et un suivi multi-fichiers (1 expérience, 2 conditions) — toutes menées de janvier à février 2026 dans le cadre du projet Aletheia. L'ensemble des données d'expérience, des fichiers de vérité terrain et des grilles de notation est disponible dans le répertoire de recherche du projet.*
