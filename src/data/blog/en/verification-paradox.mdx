...
summary: "Our priciest model scored 6/12 on code review. A mid-tier model plus one instruction scored 11/12. Discipline beat compute."
pubDate: 2026-02-18
author: "Ivan Vetrov"
lang: "en"
slug: "verification-paradox"
cover:
  src: "/images/blog/verification-paradox.webp"
...
## More Agents, Worse Results — on Single-File Tasks

This was the uncomfortable finding on single-file tasks.
...
**The picture inverts when tasks span multiple files.** A follow-up experiment reviewed a three-script production pipeline — signal detection, draft generation, and publishing — with genuine inter-file data flow. The result reversed: Team 9/12, Solo 7/12, a +2 delta in the team's favor. Solo agents degraded from 11/12 on a single file to 7/12 on three files — a −4 complexity penalty that teams absorbed through file specialization. Each agent owned one component and surfaced cross-script exploitation chains that a single context window missed: the team found a cross-script injection path that solo identified only as a key mismatch, without tracing the downstream consequence.

The crossover appears at roughly three files with genuine inter-file data flow. Below that threshold, teams add coordination overhead without parallelism payoff. Above it, file specialization justifies the cost.
...
No agent swarms for single-file tasks — teams become worthwhile at three or more files with genuine inter-file interfaces. No skill injection. No expensive models. This scored 11–12/12 on every task type.

## What We Got Wrong

Our predictions were 4 out of 11 correct. We expected verification to help everywhere equally — it's synergistic with structure, not additive. We expected teams to outperform solo agents — they don't on single-file judgment tasks, but they do on multi-file tasks with inter-file dependencies. The structure-verification synergy was completely unpredicted. It emerged from the data, not from theory.
...
*This article draws on four experiment series: H-MATRIX (23 experiments), prior VERIFY research (36 experiments), hypothesis validation (27 experiments), and a multi-file follow-up (1 experiment, 2 conditions) — all conducted in February 2026. Experiment data, ground truth files, and scoring rubrics are available in the project's research directory. Methodology: [From 13/19 to 19/19: Hypothesis-Driven Development for AI Systems](/articles/hypothesis-driven-development-ai-systems).*
